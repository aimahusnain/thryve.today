...
export default function Dashboard() {
  ...
  const adminPercentage = data?.usersByRole?.find((r: { role: string; _count: { _all: number } }) => r.role === "ADMIN")?._count._all || 0;
  const userPercentage = data?.usersByRole?.find((r: { role: string; _count: { _all: number } }) => r.role === "USER")?._count._all || 0;
                    ...
                    <CardContent>
                      <div className="h-[200px] w-full">
                        <div className="flex h-full items-end gap-2">
                          {[
                            { label: "Pending", value: data?.payments.pending || 0, color: "bg-amber-400" },
                            { label: "Completed", value: data?.payments.completed || 0, color: "bg-green-500" },
                            { label: "Failed", value: data?.payments.failed || 0, color: "bg-red-500" },
                          ].map((item, i) => {
                      ...
                      <div className="relative flex h-44 w-44 items-center justify-center">
                        <svg className="h-full w-full" viewBox="0 0 100 100">
                          ...
                          <circle
                            ...
                            strokeDashoffset={251.2 - ((data?.counts.enrollments || 0) / 100) * 251.2}
